...
![流程图 (1).jpg](https://p-bytetech.bytedance.net/tos-cn-i-vz0z6vmpra/c8decf2cd9ea4554b75aefe3c6e96d73~tplv-vz0z6vmpra-image.image)
...
- 默认约定配置文件为 `config.default.ts` ，默认环境变量 `CONF_SERVER_ENV` ，可以通过 hook 来定制相关的环境变量解析方式。
- 根据环境变量 `env` 来加载对应 `config.${env}.ts` 配置文件
- `config.${env}.ts` 将采用[deepmerge](https://github.com/TehShrike/deepmerge) 的方式进行合并，值得注意的是对于数组项的合并，框架采用了直接替换的方式，因为数组元素之间往往存在顺序的要求，直接合并可能会导致与预期不符的结果。例如 环境变量为`CONF_SERVER_ENV=prod` ，`config.prod.ts` 将会覆盖并合并 `config.default.ts`
...
![lib/loader/file_loader.js](https://tech-proxy.bytedance.net/tos/images/1645759293924_b2e74a9aade2892fb4fc8ce4b8554475)

[lib/loader/mixin/controller.js](https://github.com/eggjs/egg-core/blob/master/lib/loader/file_loader.js)

![lib/loader/mixin/controller.js](https://tech-proxy.bytedance.net/tos/images/1645759294367_24a1626c4d2b2aced28f815faf6d8531)

[lib/loader/context_loader.js](https://github.com/eggjs/egg-core/blob/master/lib/loader/context_loader.js#L92)

![lib/loader/context_loader.js](https://tech-proxy.bytedance.net/tos/images/1645759293813_dd109ad7270df3aba61a7a3a7b15f14b)

- 以注入形式使用 IoC 托管的类实例（TypeScript）

```typescript
// src/app/controller/home.ts
import { Inject, Injectable } from '@conf-js/a-framework';
