...
```ts title="module-api/api.service.ts" {3}
...
```ts title="module-api/api.contoller.ts" {7-8,13}
import { Inject, GET, Controller, ContentType } from '@artusx/core';
import type { ArtusXContext } from '@artusx/core';
import APIService from './api.service';
...
```ts title="middleware/LimitRate.ts" copy
...
```ts title="middleware/traceTime.ts" copy
...
```ts title="controller/home.ts" copy {7}
...
```ts title="shedule/notify.ts" {4-8}
