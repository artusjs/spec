...
flowchart LR
    ...
    subgraph FindPath
        direction TB
        A["ScanFolderFinder<br/>implements<br/>IPathFinder"]
        B["ManifestFinder<br/>implements<br/>IPathFinder"]
    end
    subgraph Require
        direction TB
        C["Module(class)"]
    ...
    subgraph Mount
        direction TB
        E["Singleton<br/>Container"]
        F["RequestContext<br/>Container"]
    end

    %% 中间箭头与标注
    A & B --> G["PathMap<br/>(Manifest)"] --> C & D

    %% 交叉箭头 + 标注
    C -->|Set<br/>By Metadata| E
    C --> F
    D --> E
    D -->|Set<br/>By Metadata| F
